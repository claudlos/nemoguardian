...
- Adapter tests cover unsafe delete, action toggles, controversial flag, bot-message ignore, configured exclusions, mod-log output, redacted evidence, case lookup, history, stats, failure inspection, repeat-offender review, channel hotspot review, policy-rule hotspot review, Twitch action planning, and webhook auth.
- CLI tests cover offline audit case lookup, history, stats, failure inspection, repeat-offender JSON output, channel hotspot JSON output, policy-rule hotspot JSON output, and `since_hours` filters.
